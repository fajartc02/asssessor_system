...
$queryf = mysqli_query($con, "select * from t_finding_4s where fid_schedule in ($fid_pd,$fidx)");
...
while($queryconfirm2=mysqli_fetch_array($queryconfirm))
{
  ...
  $querydivisi = mysqli_query($con, "select sum(farray_value) as farray_value from t_schedule_4s where fline = '$fline' and fworsite = '$fworsite' and finfo = 'Check and Action'");
while($querydivisi2=mysqli_fetch_array($querydivisi))
{
  $farray_valuesmm = $querydivisi2['farray_value'];
 } 
...
<form action="cek_levelup_4s_qc.php" method="post" >
...
$emailBody .="<h4><b>Isi Temuan</b></h4>";
    ...
    $emailBody .="<td  width=\"5%\">No</td><td width=\"10%\">Judul</td><td width=\"7%\">Judul</td><td width=\"5%\">Item</td><td width=\"7%\">no</td><td width=\"16%\">Konten</td><td width=\"30%\">Note</td><td width=\"20%\">Temuan</td><td width=\"20%\">Tanggal</td>";
    $emailBody .="</tr>";
...
while($queryku2=mysqli_fetch_array($queryku))
{
	...
	$des = mysqli_query($con, "select * from t_form_4s_qc where fid = '$fid_score'");
	while($des2=mysqli_fetch_array($des))
{
	
	$fjudul1 = $des2['fjudul1'];
	$fjudul2 = $des2['fjudul2'];
	$fitem = $des2['fitem'];
	$fno = $des2['fno'];
	$fkonten = $des2['fkonten'];
	...
	$emailBody .="<tr>";
    $emailBody .="<td>$no</td><td>$fjudul1</td><td>$fjudul2</td><td>$fitem</td><td>$fno</td><td>$fkonten</td><td>$xml->body</td><td><img style='width:100px;' src='".LOC_IMAGE."images/temuan4s/".$fphoto."' /></td><td>$fdate_modified</td>";
    $emailBody .="</tr>";
...
$farray_value = $_POST["farray_value"];
  ...
  echo "<script>window.location='cek_board_4s_qc.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex'</script>";
...
if (isset($_POST['submit_temuan']))
{
  ...
  echo "<script>window.location='cek_board_4s_qc.php?fid=$fid_schedule&&fline=$flinex&&fworsite=$fworsitex'</script>";
...
<div class="modal" id="myModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content" >
      ...
      <form action="cek_levelup_4s.php" method="post" enctype="multipart/form-data">
   ...
   while($sub2=mysqli_fetch_array($sub))
   {
	...
	<input type="hidden" name="val<?php echo $i; ?>" id="val<?php echo $i; ?>_" />	
