...
$vals = explode(";",$fscore_nye);
	...
	$ss = mysqli_query($con, "select count(fid) as jml from t_form_stw order by fid asc");
...
$test = mysqli_query($con, "select * from t_form_stw order by fid asc");
...
<td>
   <select name="valr<?php echo $no_jml; ?>" id="valr<?php echo $no_jml; ?>" class="<?php echo str_replace(array(' ', '&', '(', ')','/'), array('', '','','',''), $fitem); ?>" onchange="<?php echo str_replace(array(' ', '&', '(', ')'), array('', '','',''), $fitem); ?>();ubahscore()" value="<?php echo $valr[$no]; ?>" style="width: 45px;">
...
if (isset($_POST['update']))
{
  ...
  $sub = mysqli_query($con, "select count(fid) as jml from t_form_stw order by fid asc");
   while($sub2=mysqli_fetch_array($sub))
   {
	$jml_form_stw = $sub2['jml']; 
   }
    
	$sum_score = 0;
	$result_array_value = "";
    $sum_array_value = "";
	for($i=1;$i<=$jml_form_stw;$i++){
...
<div class="modal" id="myModal">
  <div class="modal-dialog modal-lg">
	...
	<?php $sub = mysqli_query($con, "select count(fid) as jml from t_form_stw order by fid asc");
   while($sub2=mysqli_fetch_array($sub))
   {
	$jml_form_stw = $sub2['jml']; 
   }
...
<?php include('includes/footer.php'); ?>    
	...
	<?php $subx = mysqli_query($con, "select count(fid) as jml from t_form_stw order by fid asc");
   while($subx2=mysqli_fetch_array($subx))
   {
	$jml_form_stwx = $subx2['jml']; 
   }
    

	for($i=1;$i<=$jml_form_stwx;$i++){
		?>
