...
$ss = mysqli_query($con, "select count(fid) as jml from t_form_pm_qc where flevel = 'gold' order by fid asc");
...
<th><center>Klausul evaluasi</center></th>
...
$test = mysqli_query($con, "select * from t_form_pm_qc where flevel = 'gold' order by fid asc");
...
if (isset($_POST['update']))
{
  ...
  $fidx = $_POST["fidx"];
    ...
    $blth_now = date("Y-m");
     ...
     $sub = mysqli_query($con, "select count(fid) as jml from t_form_pm_qc where flevel = 'gold' order by fid asc");
    ...
    <div class="modal-content" >
      ...
      <form action="cek_board_pm_qc.php" method="post" enctype="multipart/form-data">
        <input type="hidden" name="fid_score" id="fid_score" >
			  	  	  	  	  	   ...
			  	  	  	  	  	   <!-- data score -->
<?php $sub = mysqli_query($con, "select count(fid) as jml from t_form_pm_qc where flevel = 'gold' order by fid asc");
   while($sub2=mysqli_fetch_array($sub))
   {
	$jml_form_om = $sub2['jml']; 
   }
  ...
  <script>
    ...
    success: function(htmlx) {
      ...
      document.getElementById('tableku').innerHTML = htmlx;
								...
								//get score
	<?php $subx = mysqli_query($con, "select count(fid) as jml from t_form_pm_qc where flevel = 'gold' order by fid asc");
