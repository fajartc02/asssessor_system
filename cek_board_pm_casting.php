...
$ss = mysqli_query($con, "select count(fid) as jml from t_form_pm_casting order by fid asc");
...
<form action="cek_board_pm_casting.php" method="post" >
...
$test = mysqli_query($con, "select * from t_form_pm_casting order by fid asc");
...
if (isset($_POST['update']))
{
 ...
 $sub = mysqli_query($con, "select count(fid) as jml from t_form_pm_casting order by fid asc");
	...
	while($des2=mysqli_fetch_array($des))
{
  ...
  mysqli_query($con, "update t_schedule_pm set farray_result = '$farray_result', farray_score = '$farray_score', fscore = '$score' where fid = '$fid_pd'");
    ...
    $fidx = $_POST["fidx"];
  $farray_resulte = $_POST["farray_result"];
  $farray_scoree = $_POST["farray_score"];
  $fid_pd = $_POST["fid_pd"];
  
  mysqli_query($con, "update t_schedule_pm set farray_result = '$farray_resulte', farray_score = '$farray_scoree', fid_pd = '$fid_pd' where fid = '$fidx'");
 ...
 echo "<script>window.location='cek_board_pm_casting.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex'</script>";
...
if (isset($_POST['submit_temuan']))
{
 ...
 $sub = mysqli_query($con, "select count(fid) as jml from t_form_pm_casting order by fid asc");
  ...
  mysqli_query($con, "update t_schedule_pm set farray_result = '$farray_result', farray_score = '$farray_score' where fid = '$fid_pd'");
  
  
  
  echo "<script>window.location='cek_board_pm_casting.php?fid=$fid_schedule&&fline=$flinex&&fworsite=$fworsitex'</script>";
...
<div class="modal" id="myModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content" >
      ...
      <form action="cek_board_pm_casting.php" method="post" enctype="multipart/form-data">
	  
	  	  <!-- data score -->

   
       <?php $sub = mysqli_query($con, "select count(fid) as jml from t_form_pm_casting order by fid asc");
   while($sub2=mysqli_fetch_array($sub))
   {
	$jml_form_pm = $sub2['jml']; 
   }
	...
	<input type="hidden" name="vals<?php echo $i; ?>" id="vals<?php echo $i; ?>_" />	
...
<?php include('includes/footer.php'); ?>    
	...
	//get score
       <?php $subx = mysqli_query($con, "select count(fid) as jml from t_form_pm_casting order by fid asc");
   while($subx2=mysqli_fetch_array($subx))
   {
	...
	for($i=1;$i<=$jml_form_pmx;$i++){
	 ...
	 document.getElementById('vals<?php echo $i; ?>_').value = document.getElementById('vals<?php echo $i; ?>').value;
