...
$regsh = mysqli_query($con, "select * from t_schedule_stw where fid_pd = '$fidschedule' and fjobas = 'Section Head'");
while ($regsh2 = mysqli_fetch_array($regsh)) {
	$cek_sh = $regsh2['fnoreg'];
}


$regmgr = mysqli_query($con, "select * from t_schedule_stw where fid_pd = '$fidschedule' and fjobas = 'Manager'");
while ($regmgr2 = mysqli_fetch_array($regmgr)) {
	$cek_mgr = $regmgr2['fnoreg'];
}


$regcross = mysqli_query($con, "select * from t_schedule_stw where fid_pd = '$fidschedule' and fjobas = 'Manager Cross'");
while ($regcross2 = mysqli_fetch_array($regcross)) {
	$cek_cross = $regcross2['fnoreg'];
}

$regdiv = mysqli_query($con, "select * from t_schedule_stw where fid_pd = '$fidschedule' and fjobas = 'Division'");
...
$tableku .= "<table class='table table-bordered dataTable table-responsive' id='dataTable' style='font-size:12px' border='0' cellspacing='0' width='100%'>
	<thead>
	<tr class='data'>
...
$queryz = mysqli_query($con, "select * from t_finding_stw where fgroup ='$fidschedule' order by fid DESC");
while ($queryz2 = mysqli_fetch_array($queryz)) {
	...
	if ($fphoto != '') {
		$foto = "<td><img style='width:100px;' src='images/temuanSTW/" . $fphoto . "' /></td>";
	} else {
		$foto = '<td></td>';
	}
