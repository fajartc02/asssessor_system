...
while($queryku2=mysqli_fetch_array($queryku))
{
	...
	if($fnoregxx == $ses_noreg){
			$test = "<a href='update_finding_pm_maintenance.php?fid=$fid&finding=$finding&fidscore=$fidscorex' class='btn btn-primary' style='font-size: 9px' >Update</a><br/><br/><a href='delete_finding_pm_maintenance.php?fid=$fid&finding=$finding&fidscore=$fidscorex&fidplan=0&fidcheck=0' onclick=\"return confirm('Apakah Anda Ingin Menghapus Data Ini ?');\" style='font-size: 9px' class='btn btn-danger text-center' data-popup='tooltip' data-placement='top' title='Hapus Data'>DELETE</a>";
		}else {
			$test = "";
		}
