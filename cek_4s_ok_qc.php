...
$queryku = mysqli_query($con, "select t_finding_4s.fid as finding, t_finding_4s.fphoto, t_finding_4s.fnote, t_finding_4s.fphoto, t_finding_4s.fdate_modified as tgl, t_schedule_4s.* from t_finding_4s join t_schedule_4s on t_finding_4s.fid_schedule =  t_schedule_4s.fid where t_finding_4s.fgroup = '$idplan' and t_finding_4s.fid_score = '$idscore' order by t_finding_4s.fdate_modified desc");
while($queryku2=mysqli_fetch_array($queryku))
{
  ...
  $ftglx = $queryku2['tgl']; 
  	...
  	if($fnoregxx == $ses_noreg){
			$test = "<a href='update_finding_4s_qc.php?fid=$fid&finding=$finding&fidscore=$fidscorex&fidplan=$fidplanx' class='btn btn-primary' style='font-size: 9px' >Update</a><br/><br/><a href='delete_finding_4s_qc.php?fid=$fid&finding=$finding&fidscore=$fidscorex&fidplan=$fidplanx&fidcheck=0' onclick=\"return confirm('Apakah Anda Ingin Menghapus Data Ini ?');\" style='font-size: 9px' class='btn btn-danger text-center' data-popup='tooltip' data-placement='top' title='Hapus Data'>DELETE</a>";
		}else {
			$test = "";
		}
