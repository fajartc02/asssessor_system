...
$queryku = mysqli_query($con, "select * from t_finding_4s where fid = '$findingx'");
...
$queryku = mysqli_query($con, "select * from t_schedule_4s where fid = '$fidx'");
while($queryku2=mysqli_fetch_array($queryku))
{
...
<div class="container-fluid p-1">
      ...
      <form action="update_finding_4s.php" method="post" enctype="multipart/form-data">
    ...
    <img src="images/temuan4s/<?php echo $fphoto; ?>" id="imgView" style="width: 30%;"class="card-img-top img-fluid">
    <hr/>
    <label>Note :</label>
...
if (isset($_POST['submit_temuan']))
{
  ...
  // Proses ubah data ke Databa 
     mysqli_query($con, "update t_finding_4s set fnote = '$fnote' where fid = '$finding'");
		...
		if($fjobas == 'Assessor'){
		echo "<script>window.location='cek_assessor_4s.php?fid=$fidx&fafteredit=1&fid_score=$fid_score'</script>";
		}else if($fjobas == 'Section Head'){
		echo "<script>window.location='cek_levelup_4s.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex&fafteredit=1&fid_score=$fid_score&fid_plan=$fid_plan'</script>";
		}else if($fjobas == 'Manager'){
		echo "<script>window.location='cek_levelup_4s.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex&fafteredit=1&fid_score=$fid_score&fid_plan=$fid_plan'</script>";
		}else if($fjobas == 'Manager Cross'){
		echo "<script>window.location='cek_levelup_4s.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex&fafteredit=1&fid_score=$fid_score&fid_plan=$fid_plan'</script>";
		}else if($fjobas == 'Division'){
		echo "<script>window.location='cek_board_4s.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex&fafteredit=1&fid_score=$fid_score&fid_plan=$fid_plan&fid_check=$fid_check'</script>";
		}
  ...
  mysqli_query($con, "update t_finding_4s set fnote = '$fnote', fphoto = '$foto' where fid = '$finding'");
