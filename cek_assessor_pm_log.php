...
$ss = mysqli_query($con, "select count(fid) as jml from t_form_pm_log order by fid asc");
...
$test = mysqli_query($con, "select * from t_form_pm_log order by fid asc");
...
if (isset($_POST['submit']))
{
  ...
  $fidx = $_POST["fidx"];
    ...
    $blth_now = date("Y-m");
   	...
   	$sub = mysqli_query($con, "select count(fid) as jml from t_form_pm_log order by fid asc");
...
while($queryku2=mysqli_fetch_array($queryku))
{
	...
	$des = mysqli_query($con, "select * from t_form_pm_log where fid = '$fid_score'");
	while($des2=mysqli_fetch_array($des))
{
  ...
  echo "<script>window.location='cek_assessor_pm_log.php?fid=$fidx'</script>";
...
if (isset($_POST['submit_temuan']))
{
  ...
  mysqli_query($con, "insert into t_finding_pm (fid_schedule, fphoto, fnote, fid_score) values ('$fid_schedule', '$foto', '$fnote', '$fid_score')");
  ...
  echo "<script>window.location='cek_assessor_pm_log.php?fid=$fid_schedule'</script>";
...
<div class="modal" id="myModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content" >
      ...
      <form action="cek_assessor_pm_log.php" method="post" enctype="multipart/form-data">
	  
	  	  	  	   <!-- data score -->

	<?php $sub = mysqli_query($con, "select count(fid) as jml from t_form_pm_log order by fid asc");
   while($sub2=mysqli_fetch_array($sub))
   {
	$jml_form_pm = $sub2['jml']; 
   }
...
<?php include('includes/footer.php'); ?>    
	...
	<?php $subx = mysqli_query($con, "select count(fid) as jml from t_form_pm_log order by fid asc");
