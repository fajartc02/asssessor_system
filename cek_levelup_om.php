...
$vals = explode(";",$fscore_nye);
	...
	$ss = mysqli_query($con, "select count(fid) as jml from t_form_om order by fid asc");
   while($ss2=mysqli_fetch_array($ss))
   {
	...
	$score = $sum_score;
	 ...
	 $scorex = mysqli_query($con, "select fscore from t_form_om order by fid asc limit 1");
...
$test = mysqli_query($con, "select * from t_form_om order by fid asc");
...
<td>
   <select name="valr<?php echo $no_jml; ?>" class="<?php echo str_replace(array(' ', '&', '(', ')'), array('', '','',''), $fjudul); ?>" onchange="<?php echo str_replace(array(' ', '&', '(', ')'), array('', '','',''), $fjudul); ?>();ubahscore()" id="valr<?php echo $no_jml; ?>" value="<?php echo $valr[$no]; ?>" style="width: 45px;">
...
<td>  
  ...
  if($awalun != $awalunz){
  ?>
  <span  class="totalscorex" id="<?php echo str_replace(array(' ', '&', '(', ')'), array('', '','',''), $fjudul); ?>"></span>
  <?php
  }
  ?>
</td>
...
$no++;  
	...
	echo "<script>
	var x = document.querySelectorAll('.".str_replace(array(' ', '&', '(', ')'), array('', '','',''), $fjudul)."');
	var total = 0;
	...
	document.getElementById('".str_replace(array(' ', '&', '(', ')'), array('', '','',''), $fjudul)."').innerHTML = text_avg;	
	
	
	
	function ".str_replace(array(' ', '&', '(', ')'), array('', '','',''), $fjudul)."(){	
	var x = document.querySelectorAll('.".str_replace(array(' ', '&', '(', ')'), array('', '','',''), $fjudul)."');
	var total = 0;
	for(var i=0;i<x.length;i++)
	{
...
if (isset($_POST['update']))
{
  ...
  $blth_now = date("Y-m");
 	...
 	$sub = mysqli_query($con, "select count(fid) as jml from t_form_om order by fid asc");
...
<div class="modal" id="myModal">
 ...
 <!-- data score -->
	...
	<?php $sub = mysqli_query($con, "select count(fid) as jml from t_form_om order by fid asc");
   while($sub2=mysqli_fetch_array($sub))
   {
	$jml_form_om = $sub2['jml']; 
   }
...
<?php include('includes/footer.php'); ?>    
  ...
  <script>
	...
	<?php $subx = mysqli_query($con, "select count(fid) as jml from t_form_om order by fid asc");
