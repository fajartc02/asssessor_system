...
while($queryku2=mysqli_fetch_array($queryku))
{
	...
	if($fnoregxx == $ses_noreg){
			$test = "<a href='update_finding_pm_casting.php?fid=$fid&finding=$finding&fidscore=$fidscorex&fidplan=$fidplanx&fidcheck=$fidcheckx' class='btn btn-primary' style='font-size: 9px' >Update</a><br/><br/><a href='delete_finding_pm_casting.php?fid=$fid&finding=$finding&fidscore=$fidscorex&fidplan=$fidplanx&fidcheck=$fidcheckx' onclick=\"return confirm('Apakah Anda Ingin Menghapus Data Ini ?');\" style='font-size: 9px' class='btn btn-danger text-center' data-popup='tooltip' data-placement='top' title='Hapus Data'>DELETE</a>";
		}else {
			$test = "";
		}
