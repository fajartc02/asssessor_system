...
$vals = explode(";",$fscore_nye);
	...
	$ss = mysqli_query($con, "select count(fid) as jml from t_form_pm_maintenance order by fid asc");
...
<form action="cek_levelup_pm_maintenance.php" method="post" >
...
$test = mysqli_query($con, "select * from t_form_pm_maintenance order by fid asc");
  ...
  $fidx = $_POST["fidx"];
   $fid_pd = $_POST["fid_pd"];
     ...
     $blth_now = date("Y-m");
  	...
  	$sub = mysqli_query($con, "select count(fid) as jml from t_form_pm_maintenance order by fid asc");
...
while($queryku2=mysqli_fetch_array($queryku))
{
	...
	$des = mysqli_query($con, "select * from t_form_pm_maintenance where fid = '$fid_score'");
	while($des2=mysqli_fetch_array($des))
{
 ...
 echo "<script>window.location='cek_levelup_pm_maintenance.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex'</script>";
...
if (isset($_POST['submit_temuan']))
{
  ...
  echo "<script>window.location='cek_levelup_pm_maintenance.php?fid=$fid_schedule&&fline=$flinex&&fworsite=$fworsitex'</script>";
...
<div class="modal" id="myModal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content" >
      ...
      <form action="cek_levelup_pm_maintenance.php" method="post" enctype="multipart/form-data">
	  
	  
	  			  	  	  	   <!-- data score -->
	<?php $sub = mysqli_query($con, "select count(fid) as jml from t_form_pm_maintenance order by fid asc");
   while($sub2=mysqli_fetch_array($sub))
   {
	$jml_form_pm = $sub2['jml']; 
   }
...
<?php include('includes/footer.php'); ?>    
  ...
  <script>
	...
	<?php $subx = mysqli_query($con, "select count(fid) as jml from t_form_pm_maintenance order by fid asc");
