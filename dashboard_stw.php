...
<div class="container-fluid p-1">
	...
	<center>
		<h1 class="h3 mb-0 text-gray-800">Dashboard STW</h1>
	</center><br>

	<div class="row">
		<div class="col-md-6"></div>
		<div class="col-md-6" style="text-align:right;color:black">Guidance Ok of STW : <a href="guidance_pdf.php?ffile=guidance_STW.pdf&link_back=dashboard_STW.php">guidance_STW.pdf (click)</a></div>
	</div>
			...
			<div class="table-responsive">
				<table class="table table-bordered" width="100%" cellspacing="0">
					...
					<tbody>
						<?php
						if ($regnox == 'admin') {
							$queryku = mysqli_query($con, " select fid, right(fdate, 2) as ftgl, date_format(fdate, '%W') as fhr, fname, finfo, fline, fworsite, farray_score from t_schedule_stw order by fdate desc limit 80");
						} else {
							$queryku = mysqli_query($con, " select fid, right(fdate, 2) as ftgl, date_format(fdate, '%W') as fhr, fname, finfo, fline, fworsite, farray_score from t_schedule_stw where fnoreg = '$regnox' order by fdate desc limit 80");
						}
						...
						while ($queryku2 = mysqli_fetch_array($queryku)) {
							...
							if ($finfo == "Plan and Do" && $fworsite == "RNR Quality Control") {
								$link = "cek_assessor_stw_qc.php?fid=$fid";
							} elseif ($finfo == "Plan and Do") {
								$link = "cek_assessor_stw.php?fid=$fid";
							} elseif ($finfo == "Check and Action" && $fworsite == "RNR Quality Control") {
								$link = "cek_levelup_stw_qc.php?fid=$fid&&fline=$fline&&fworsite=$fworsite";
							} elseif ($finfo == "Check and Action") {
								$link = "cek_levelup_stw.php?fid=$fid&&fline=$fline&&fworsite=$fworsite";
							} else {
								$link = "cek_board_stw.php?fid=$fid&&fline=$fline&&fworsite=$fworsite";
							}
...
/*
						if ($regnox == 'admin') {
							$queryku = mysqli_query($con, "select t_finding_stw.fid_schedule, t_finding_stw.fnote, t_finding_stw.fstatus, date(t_finding_stw.fdate_modified) as fdate, t_schedule_stw.finfo, t_schedule_stw.fnoreg, t_schedule_stw.fname, t_schedule_stw.fworsite, DAYNAME(date(t_finding_stw.fdate_modified)) as fday from t_finding_stw join t_schedule_stw on t_schedule_stw.fid = t_finding_stw.fid_schedule order by t_finding_stw.fid desc");
						} else {
...
//}
			...
			while ($queryku2 = mysqli_fetch_array($queryku)) {
				...
				$fday = $queryku2['fday'];
					...
					if ($finfo == "Plan and Do" && $fworsite == "RNR Quality Control") {
								$link = "cek_detail_stw_qc.php?fid=$fid";
							}elseif ($finfo == "Plan and Do") {
								$link = "cek_detail_stw.php?fid=$fid";
							}


				$query =  mysqli_query($con, "SELECT COUNT(fgroup) as hitung FROM t_finding_stw where fgroup = '$fid'");
				while ($query2 = mysqli_fetch_array($query)) {
				$finding = $query2['hitung'];

				}

				$queryoke =  mysqli_query($con, "SELECT fscore FROM t_schedule_stw where fid = '$fid'");
				...
				// }
					...
					$query_acc =  mysqli_query($con, "SELECT t_schedule_stw.*, t_pattern_schedule.fname from t_schedule_stw  join t_pattern_schedule on t_schedule_stw.fnoreg = t_pattern_schedule.fnoreg where t_pattern_schedule.fjobas = 'Assessor' and t_schedule_stw.fid = '$fid' and t_schedule_stw.fworsite = '$fworsite' and t_pattern_schedule.ftype_pillar = 'STW'");
...
function mySolving(fid) {
    ...
    type: 'POST',
    data: dataString,
    url: 'status_dashboard_stw_os.php',       
