...
if (isset($_POST['submit_temuan']))
{
  ...
  // Proses ubah data ke Databa 
	...
	if($fjobas == 'Assessor'){
		echo "<script>window.location='cek_assessor_pm_qc.php?fid=$fidx&fafteredit=1&fid_score=$fid_score'</script>";
		}else if($fjobas == 'Section Head'){
		echo "<script>window.location='cek_levelup_pm_qc.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex&fafteredit=1&fid_score=$fid_score&fid_plan=$fid_plan'</script>";
		}else if($fjobas == 'Manager'){
		echo "<script>window.location='cek_levelup_pm_qc.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex&fafteredit=1&fid_score=$fid_score&fid_plan=$fid_plan'</script>";
		}else if($fjobas == 'Manager Cross'){
		echo "<script>window.location='cek_levelup_pm_qc.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex&fafteredit=1&fid_score=$fid_score&fid_plan=$fid_plan'</script>";
		}else if($fjobas == 'Division'){
		echo "<script>window.location='cek_board_pm_qc.php?fid=$fidx&&fline=$flinex&&fworsite=$fworsitex&fafteredit=1&fid_score=$fid_score&fid_plan=$fid_plan&fid_check=$fid_check'</script>";
		}
